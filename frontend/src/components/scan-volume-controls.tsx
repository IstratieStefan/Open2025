...
interface ScanVolumeControlsProps {
  ...
  height?: number
  depth?: number
  onVolumeChange?: (volume: { width: number; height: number; depth: number }) => void
  onReset?: () => void
  minSize?: number
  maxSize?: number
}
...
export function ScanVolumeControls({
  ...
  const handleSliderChange = (dimension: 'width' | 'height' | 'depth') => (values: number[]) => {
    updateDimension(dimension, values[0])
  }

  const handleInputChange = (dimension: 'width' | 'height' | 'depth') => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 p-3 bg-muted/50 rounded-lg">
          ...
          <div className="text-center">
            <div className="text-lg font-semibold">{width}×{height}×{depth}</div>
            <div className="text-sm text-muted-foreground">W×H×D (cm)</div>
          </div>
        </div>
        ...
        {(width * height * depth) > 1000 && (
          <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-md">
            <p className="text-sm text-amber-700 dark:text-amber-400">
              Large scan volumes may require longer scan times and more storage space.
            </p>
          </div>
        )}
      </CardContent>
